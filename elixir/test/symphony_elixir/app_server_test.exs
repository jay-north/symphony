defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server executes supported dynamic tool calls and returns the tool result" do
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case \"$count\" in
          ...
          4)
            ...
            printf '%s\\n' '{\"id\":102,\"method\":\"item/tool/call\",\"params\":{\"name\":\"linear_update_issue_state\",\"callId\":\"call-90a\",\"threadId\":\"thread-90a\",\"turnId\":\"turn-90a\",\"arguments\":{\"issue_id\":\"DEN-53\",\"state\":\"Backlog\"}}}'
            ;;
          5)
            printf '%s\\n' '{\"method\":\"turn/completed\"}'
            exit 0
      ...
      assert_received {:tool_called, "linear_update_issue_state",
                       %{
                         "issue_id" => "DEN-53",
                         "state" => "Backlog"
                       }}
  ...
  test "app server emits tool_call_failed for supported tool failures" do
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case \"$count\" in
          ...
          4)
            ...
            printf '%s\\n' '{\"id\":103,\"method\":\"item/tool/call\",\"params\":{\"tool\":\"linear_update_issue_state\",\"callId\":\"call-90b\",\"threadId\":\"thread-90b\",\"turnId\":\"turn-90b\",\"arguments\":{\"issue_id\":\"DEN-53\",\"state\":\"Backlog\"}}}'
            ;;
          5)
            printf '%s\\n' '{\"method\":\"turn/completed\"}'
            exit 0
      ...
      assert_received {:tool_called, "linear_update_issue_state", %{"issue_id" => "DEN-53", "state" => "Backlog"}}

      assert_received {:app_server_message, %{event: :tool_call_failed, payload: %{"params" => %{"tool" => "linear_update_issue_state"}}}}
    after
      File.rm_rf(test_root)
    end
  end
